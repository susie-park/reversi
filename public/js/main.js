...
socket.on('game_update',function(payload){
  ...
  if('undefined' == typeof board || !board){
    console.log('Internal error: received a malformed board update from the sever');
    return;
  }
  ...
  else if(socket.id == payload.game.player_pink.socket){
      my_color = 'pink';
  }
  else{
    ...
    for(column = 0; column < 8; column++){
      ...
      if(old_board[row][column] != board[row][column]){
        if(old_board[row][column] == '?' && board[row][column] == ' '){
          ...
          $('#'+row+'_'+column).html('<img src="assets/images/green_to_pink.gif" alt="pink square" />');
        }
        else if(old_board[row][column] == 'p' && board[row][column] == 'g'){
          $('#'+row+'_'+column).html('<img src="assets/images/pink_to_green.gif" alt="green square" />');
        }
        else{
